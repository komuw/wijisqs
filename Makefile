upload:
	@rm -rf build
	@rm -rf dist
	@sudo rm -rf wijisqs.egg-info
	@python setup.py sdist
	@python setup.py bdist_wheel
	@twine upload dist/* -r testpypi
	@pip install -U -i https://testpypi.python.org/pypi wijisqs


VERSION_STRING=$$(cat wijisqs/__version__.py | grep "__version__" | sed -e 's/"__version__"://' | sed -e 's/,//g' | sed -e 's/"//g' | sed -e 's/ //g')
uploadprod:
	@rm -rf build
	@rm -rf dist
	@sudo rm -rf wijisqs.egg-info
	...
	@printf "\n with commit message, see Changelong: https://github.com/komuw/wijisqs/blob/master/CHANGELOG.md \n" && git tag -a "$(VERSION_STRING)" -m "see Changelong: https://github.com/komuw/wijisqs/blob/master/CHANGELOG.md"
	@printf "\n git push the tag::\n" && git push --all -u --follow-tags
	@pip install -U wijisqs
...
test:
	...
	@printf "\n coverage report html::\n" && coverage html --fail-under=84 --title=wijisqs_coverage
	@printf "\n run flake8::\n" && flake8 .
	@printf "\n run pylint::\n" && pylint --enable=E --disable=W,R,C --unsafe-load-any-extension=y examples/ wijisqs/ tests/ documentation/
	@printf "\n run bandit::\n" && bandit -r --exclude .venv -ll .
	@printf "\n run mypy::\n" && mypy --show-column-numbers --strict wijisqs/
